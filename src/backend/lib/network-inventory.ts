...
import { vendorForMac, type RegisteredDevice, type SeenHostRow } from './network-discovery';
...
export function crossReference(input: CrossRefInput): DiscoveryReport {
  ...
  for (const entry of input.registered) {
    // Sem endereço não ocupa endereço nenhum. O equipamento em DHCP por
    // registar existe, mas quem o encontra é a reconciliação pelo MAC — aqui
